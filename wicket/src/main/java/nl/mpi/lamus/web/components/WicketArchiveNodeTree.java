...
/**
 *
 * Adapted from {@link org.apache.wicket.extensions.markup.html.tree.Tree} and 
 * {@link org.apache.wicket.extensions.markup.html.tree.DefaultAbstractTree}
 *
 * @author Twan Goosen <twan.goosen@mpi.nl>
 */
public class WicketArchiveNodeTree extends AbstractTree {
     ...
     * The component id
     */
    public WicketArchiveNodeTree(String id) {
	super(id);
	init();
    }
     ...
     * The tree model
     */
    public WicketArchiveNodeTree(String id, IModel<TreeModel> model) {
	super(id, model);
	init();
    }
    ...
    public WicketArchiveNodeTree(String id, TreeModel model) {
	...
	final ArchiveNode node = (ArchiveNode) item.getDefaultModelObject();

	item.add(newIndentation(item, "indent", (ArchiveNode) item.getDefaultModelObject(), level));
     ...
     */
    protected String renderNode(ArchiveNode node) {
     ...
     */
    private static final ResourceReference CSS = new ResourceReference(WicketArchiveNodeTree.class, "tree/tree.css");
    /** Reference to the icon of closed tree folder */
    private static final ResourceReference FOLDER_CLOSED = new ResourceReference(WicketArchiveNodeTree.class, "tree/folder-closed.gif");
    /** Reference to the icon of open tree folder */
    private static final ResourceReference FOLDER_OPEN = new ResourceReference(WicketArchiveNodeTree.class, "tree/folder-open.gif");
    /** Reference to the icon of tree item (not a folder) */
    private static final ResourceReference ITEM = new ResourceReference(WicketArchiveNodeTree.class, "tree/item.gif");
     ...
     * @return The package resource reference
     */
    protected ResourceReference getNodeIcon(ArchiveNode node) {
     ...
     */
    protected Component newIndentation(MarkupContainer parent, String id, final ArchiveNode node,
	    final int level) {
	WebMarkupContainer result = new WebMarkupContainer(id) {
		...
		ArchiveNode parent = node.getParent();
    ...
    protected Component newJunctionLink(MarkupContainer parent, final String id,
	    final String imageId, final ArchiveNode node) {
     ...
     * {@link #getNodeIcon(nl.mpi.archiving.tree.ArchiveNode) }.
     *
     * @param parent
     * The parent component
     * @param id
     ...
     */
    protected Component newNodeIcon(MarkupContainer parent, String id, final ArchiveNode node) {
     ...
     */
    protected MarkupContainer newNodeLink(MarkupContainer parent, String id, final ArchiveNode node) {
     ...
     */
    protected void onJunctionLinkClicked(AjaxRequestTarget target, ArchiveNode node) {
    }
     ...
     */
    private boolean isNodeLast(ArchiveNode node) {
	ArchiveNode parent = node.getParent();
	if (parent == null) {
	    return true;
	} else {
	    return parent.getChild(parent.getChildCount() - 1).equals(node);
	}
    }
}
