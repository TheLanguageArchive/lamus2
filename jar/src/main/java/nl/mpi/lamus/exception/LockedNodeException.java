 ...
 * @author guisil
 */
public class LockedNodeException extends NodeAccessException {
    
    private static String messagePart1 = "Node with URI '";
    private static String messagePart2 = "' is already locked by ";
    
    private int workspaceID;
    
    public LockedNodeException(URI nodeURI, int workspaceID) {
        super(getConstructedMessage(nodeURI.toString(), workspaceID), nodeURI, null);
        this.workspaceID = workspaceID;
    }
    
    public LockedNodeException(String message, URI nodeURI, int workspaceID) {
    ...
    private static String getConstructedMessage(String nodeUriStr, int workspaceID) {
        ...
        if(workspaceID > -1) {
            builder.append("workspace ").append(workspaceID);
        } else {
            builder.append("multiple workspaces");
        }
